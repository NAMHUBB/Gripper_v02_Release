// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  스크립트 러너 — 현재 비활성화(전체 주석 처리)                              ║
// ║                                                                          ║
// ║  Dashboard 화면에서 스크립트 편집기를 제거하면서 이 모듈도 통째로 주석      ║
// ║  처리했습니다. 다시 사용하려면:                                            ║
// ║    1) 아래 라인들의 선행 `// ` 를 제거                                      ║
// ║    2) 상단 import 중 getRegCache/setRegCache 정의 블록은 삭제하고           ║
// ║       `import { getRegCache, setRegCache } from './regCache'` 로 대체       ║
// ║       (레지스터 캐시는 src/regCache.ts 로 분리되어 계속 사용 중)             ║
// ║    3) DashboardTab.tsx 하단의 ScriptPanel 주석 블록을 복원                  ║
// ║    4) 파일 맨 아래의 `export {};` 삭제                                      ║
// ╚══════════════════════════════════════════════════════════════════════════╝
//
...
// isolatedModules 환경에서 빈 파일이 전역 스크립트로 취급되지 않도록 유지
export {};
